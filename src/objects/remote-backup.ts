...
class RemoteBackup {
  ...
  request({ path, options = {}, body, query, onbody }) {
    // console.log(this.protocol, this.host, this.port, path, options, body, query, onbody);
    return new Promise((resolve, reject) => {
      ...
      const request = this.http.request(options, res => {
        ...
        res.on('error', (err) => {
          console.error(err);
          console.log('calling body.destroy');
          // body.destroy();     // TOOD: This is wrong?
          reject(err)
        });
      });
